...
export default function App() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!raw) return;
      const saved = JSON.parse(raw) as { email: string; role: string } | null;
      if (saved) {
        setUser(saved);
  ...
  const handleLogin = (email:string, pass:string) => {
    ...
    if(normalizedEmail === "admin@risol.com" && password === "admin123"){ 
      setUser({ email: normalizedEmail, role: "admin" }); 
      setView("admin");
      return;
    }

    if(normalizedEmail === "karyawan@risol.com" && password === "karyawan123"){ 
      setUser({ email: normalizedEmail, role: "karyawan" }); 
      setView("karyawan");
      return;
    }
            ...
            <section ref={aboutRef} className="scroll-mt-24">
              <div className="rounded-[28px] overflow-hidden bg-white border shadow-[0_12px_40px_rgba(74,44,42,0.08)] grid md:grid-cols-[1.1fr_0.9fr]">
                ...
                <div className="relative min-h-[380px] bg-[#FFE9D2]">
                  <img src="https://images.unsplash.com/photo-1604908177453-7462950a6a3b?q=80&w=800" className="absolute inset-0 w-full h-full object-cover" alt="Risol"/>
